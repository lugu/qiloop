...
flags_F = ProtoField.uint16("qim.flags","Flags")
...
function qim_proto.dissector(buffer,pinfo,tree)
  ...
  if tostring(magic) == "42dead42" then
    ...
    local servicen = service:le_uint() + 1
    services = { "ServiceServer", "ServiceDirectory" }

    if services[servicen] ~= nil then
        subtree:add(service_desc_F, services[servicen])
    end
    ...
    local actionn = action:le_uint() + 1
    actions = { "RegisterEvent", "UnregisterEvent", "MetaObject", "Terminate", "GetProperty",
        "SetProperty", "Properties", "RegisterEventWithSignature" }

    if actionn == 9 and servicen == 1 then
        subtree:add(action_desc_F, "Authenticate")
    elseif actions[actionn] ~= nil then
        subtree:add(action_desc_F, actions[actionn])
    end
